...
const createAccount = (name, surname) => {
    bank.push({
        name: name,
        surname: surname,
        account: 'LT' + Math.floor (Math.random()*1000000000),
        money: 0,
    })
...
createAccount('aliux', 'butux');
createAccount('zilva', 'pilva');
createAccount('ryte', 'lete');



updateAccount(bank[0].account, 99)
...
updateAccount(bank[0].account, -5);
updateAccount(bank[1].account, -888);
updateAccount(bank[2].account, -81321315);
